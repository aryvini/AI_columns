...
pilares = pd.read_excel("Pilares.xlsx",sheet_name="Pilares")
seu_nome = pilares.Seu_nome[0]

##cria um txt com seu nome abre e fecha, garantindo que um antigo arquivo seja limpado
open(seu_nome+".txt","w+").close()

logf = open(seu_nome+".txt", "a", encoding='utf-8')
sys.stdout = logf


print(f"COMEÇANDO O RELATÓRIO DE:")
print(seu_nome)
print(f"\n\n")

qtde = len(pilares)

for pilar in range(0,qtde,1):
    ...
    Nsk = pilares.Nk[pilar]
    Msk_y_topo = pilares.Mytopo[pilar]
    Msk_y_base = pilares.Mybase[pilar]
    Msk_x_topo = pilares.Mxtopo[pilar]
    Msk_x_base = pilares.Mxbase[pilar]
    ...
    PDE = pilares.PDE[pilar]
    viga_x = pilares.Viga_x[pilar]
    viga_y = pilares.Viga_y[pilar]
    delta_c = pilares.Dc[pilar]
    caa = pilares.CAA[pilar]
    ...
    # Retangular
        ...
        while True:
            if b1 == 'n':
                ...
                b = pilares.Base[pilar].astype(str)
        ...
        while True:
            h1 = 's' ##fixa sempre a altura do pilar
        ...
        while True:
            if h1 == 'n':
                ...
                h = pilares.Altura[pilar].astype(str)
    ...
    if geometria[0] == 'r':
        ...
        for i1 in range(0, len(tabela_solucoes_possiveis)):
            ...
            if numero_verificacoes == len(situacoes_de_calculo):
                ...
                #logf.write(f'\n\033[1;34m{"2.2 Solução mais econômica"}\033[m \n')
                print(f'{"1 - Resistencia e dados da seção"}')
                #logf.write(f'{"1 - Resistencia e dados da seção"}')
                print(f"  • fck = {fck * 10} MPa\n  • Øpilar = {fi_pilar} cm\n  • C = {cobrimento} cm;")
                #logf.write(f"  • fck = {fck * 10} MPa\n  • Øpilar = {fi_pilar} cm\n  • C = {cobrimento} cm;")
                print(f'\n{"2 - Dados da armadura"}')
                #logf.write(f'\n{"2 - Dados da armadura"}')
                print(f"  • Øt = {fi_t} mm\n  • Øl = {fi_l} mm\n"
                    f"  • Nc = {Nc}\n"
